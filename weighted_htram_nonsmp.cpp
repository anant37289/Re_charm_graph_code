...
public:
  ...
  Main(CkArgMsg *m) {
    ...
    while (getline(file, readbuf)) {
      ...
      // Edge length " << edge_distance << endl; find the maximum vertex index
    ...
    tram_proxy = tram_proxy_t::ckNew(updater_array_gid, buffer_size,
                                     enable_buffer_flushing, flush_timer);
    mainProxy = thisProxy;
  ...
  void check_buffer_done(int *msg_stats, int N) {
    // ckout << "Receives: " << msg_stats[1] << ", Sends: " << msg_stats[0] <<
    // endl;
...
public:
  ...
  void get_graph(LongEdge *edges, int E, int *partition, int dividers) {
    ...
    if (num_vertices != 0) {
      ...
      for (int i = 0; i < E; i++) {
        ...
        // endl;
        local_graph[edges[i].begin - start_vertex].adjacent.insertAtEnd(
            new_edge);
      }
    }
    int now_done = 1;
  ...
  static void
  update_distance_caller(void *p, std::pair<int, int> new_vertex_and_distance) {
  ...
  void update_distances_local() {
    ...
    while (pq.size() > 0) {
      ...
      pq.pop();
      if (new_vertex_and_distance.first >= partition_index[thisIndex] &&
          new_vertex_and_distance.first < partition_index[thisIndex + 1]) {
        ...
        // if (CkMyPe()==1) ckout << "Incoming local pair on PE " << thisIndex
        // << ": " << new_vertex_and_distance.first << ", " <<
        // new_vertex_and_distance.second << endl;
        //  if the incoming distance is actually smaller
        if (new_vertex_and_distance.second <
            local_graph[local_index].distance) {
          local_graph[local_index].distance =
              new_vertex_and_distance.second; // update distance
          // for all neighbors
          for (int i = 0; i < local_graph[local_index].adjacent.size(); i++) {
            ...
            for (int j = 0; j < N; j++) {
              ...
              if (updated_dist.first >= partition_index[j] &&
                  updated_dist.first < partition_index[j + 1]) {
                dest_proc = j;
                break;
              }
              if (j == N - 1)
            ...
            if (updated_dist.first > 0 && updated_dist.first < V) {
              // send buffer to pe
              if (dest_proc == CkMyPe()) {
                // if (CkMyPe()==1) ckout << "Outgoing local pair on PE " <<
                // thisIndex << ": " << updated_dist.first << ", " <<
                // updated_dist.second << endl;
                pq.push(updated_dist);
              } else {
                // if (CkMyPe()==1) ckout << "Outgoing remote pair on PE " <<
                // thisIndex << ": " << updated_dist.first << ", " <<
                // updated_dist.second << endl;
  ...
  /**
   * Update distance. Consumes a vertex and a new distance, potentially updates
   * the distance, and then keeps going
   */
  void update_distances(std::pair<int, int> new_vertex_and_distance) {
    ...
    if (new_vertex_and_distance.first >= partition_index[thisIndex] &&
      ...
      // if (CkMyPe()==1) ckout << "Incoming remote pair on PE " << thisIndex <<
      // ": " << new_vertex_and_distance.first << ", " <<
      // new_vertex_and_distance.second << endl;
      //  if the incoming distance is actually smaller
