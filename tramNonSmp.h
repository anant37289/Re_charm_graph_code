...
template <typename T, typename SingletonClass> class singleton {
public:
  using value_type = T;
...
template <typename T> struct tramNonSmpMsg : public CMessage_tramNonSmpMsg<T> {
...
template <typename T> tramNonSmpMsg<T> *make_tram_msg(int size_) {
...
template <typename T> class tramNonSmp : public CBase_tramNonSmp<T> {
private:
  using value_type = T;
...
template <typename T> tramNonSmp<T>::tramNonSmp(CkMigrateMessage *msg) {}
...
template <typename T>
tramNonSmp<T>::tramNonSmp(CkGroupID gid, int buffer_size_,
                          bool enable_buffer_flushing, double time_in_ms)
    : func_ptr(nullptr), obj_ptr(nullptr), is_itemized(true),
...
template <typename T> void periodic_progress(void *htram_obj, double time) {
...
template <typename T> void tramNonSmp<T>::num_flushes() {
  ckout << "[PE:" << CkMyPe() << "] Flush Counter: " << flush_counter << endl;
}

template <typename T> void tramNonSmp<T>::register_flush() {
...
template <typename T> void tramNonSmp<T>::set_itemized(bool value) {
  is_itemized = value;
}
...
template <typename T> void tramNonSmp<T>::periodic_flush() {
  buffer_t &buffer_size = TRAM_ACCESS_SINGLETON(payload_buffer_size);
...
template <typename T> void tramNonSmp<T>::tflush() {
  buffer_t &buffer_size = TRAM_ACCESS_SINGLETON(payload_buffer_size);
...
template <typename T> void tramNonSmp<T>::receive(tramNonSmpMsg<T> *msg) {
